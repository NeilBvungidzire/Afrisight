...
@section('title', 'Admin' . ' - ' . 'Update screening question' . ' - ' . config('app.name'))

@section('content')
    <h1>Update screening question</h1>

    @alert

    <form action="{{ route('admin.screening.update-question', ['id' => $question->id]) }}" method="post">
        @csrf
        @method('put')
        ...
        @php($key = $attribute)
        @php($value = old($attribute) ?? $question->question)
        ...
        @php($key = $attribute)
        @php($value = old($attribute) ?? $question->question_type)
        <div class="form-group row">
        ...
        @php($key = 'params.info')
        @php($value = old($attribute) ?? $question->params['info'] ?? '')
        <div class="form-group row">
        ...
        @php($value = old($attribute) ?? $question->params['country_code'] ?? '')
        <div class="form-group row">
