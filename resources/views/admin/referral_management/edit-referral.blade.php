...
@section('content')
    ...
    <form action="{{ route('admin.referral_management.update_referral', ['id' => $referral->id]) }}" method="post" class="form">
        @csrf
        @method('put')

        @php
            $fieldName = 'referrer_type';
            $value = old($fieldName) ?? $referral->referrerable_type;
        @endphp
        ...
        @php
            ...
            $value = old($fieldName) ?? $referral->referrerable_id;
        @endphp
        <div class="form-group row">
            <label for="{{ $fieldName }}" class="col-sm-5 col-lg-4 col-form-label">
                Referrer ID
        ...
        @php
            ...
            $value = old($fieldName) ?? $referral->amount_per_successful_referral;
        @endphp
        <div class="form-group row">
            <label for="{{ $fieldName }}" class="col-sm-5 col-lg-4 col-form-label">
                Amount (USD)
        ...
        @php
            $fieldName = 'type';
            $value = old($fieldName) ?? $referral->type;
        @endphp
        <div class="form-group row">
            <label for="{{ $fieldName }}" class="col-sm-5 col-lg-4 col-form-label">
                Type
        ...
        @if(isset($type) && $type === \App\Constants\ReferralType::RESPONDENT_RECRUITMENT)
            @php
                ...
                $value = old($fieldName) ?? $referral->data['project_code'] ?? '';
            @endphp
            <div class="form-group row">
                <label for="{{ $fieldName }}" class="col-sm-5 col-lg-4 col-form-label">
                    Project Code
        ...
        @php
            $fieldName = 'code';
            $value = old($fieldName) ?? $referral->code;
        @endphp
        <div class="form-group row">
            <label for="{{ $fieldName }}" class="col-sm-5 col-lg-4 col-form-label">
                Referral Code
        ...
        @php
            ...
            $value = old($fieldName) ?? $referral->data['public_reference'] ?? '';
        @endphp
        <div class="form-group row">
            <label for="{{ $fieldName }}" class="col-sm-5 col-lg-4 col-form-label">
