...
@section('title', 'Admin' . ' - ' . 'Edit project' . ' - ' . config('app.name'))

@section('content')
    <h1>Edit project <span class="badge badge-info">{{ $project->code }}</span></h1>
    ...
    <form action="{{ route('admin.projects.update', ['id' => $project->id]) }}" method="post">
        ...
        @php($value = old($attribute) ?? $project->client_id)
        <div class="form-group row">
            <label class="col-sm-5 col-lg-4 col-form-label" for="{{ $attribute }}">Client</label>

            <div class="col-sm-7 col-lg-8">
                <select id="{{ $attribute }}" name="{{ $attribute }}" required autofocus
        ...
        @php($value = old($attribute) ?? $project->targets_definition[\App\Services\ProjectManagement\Constants\TargetCriteria::COUNTRY][0] ?? null)
        <div class="form-group row">
        ...
        {{-- Project code --}}
        @php($attribute = 'code')
        @php($value = old($attribute) ?? $project->code)
        <div class="form-group row">
        ...
        @php($value = old($attribute) ?? $project->description)
        <div class="form-group row">
                ...
                @foreach($channels as $channel)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" value="{{ $channel }}" id="{{ $channel }}"
                               name="{{ $attribute }}[]"{{ $project->channels[$channel] ? ' checked' : '' }}>
                ...
                @foreach($languages as $language)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" value="{{ $language }}" id="{{ $language }}"
                               name="{{ $attribute }}[]"{{ in_array($language, (array)$project->language_restrictions) ? ' checked' : '' }}>
                ...
                @foreach($devices as $device)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" value="{{ $device }}" id="{{ $device }}"
                               name="{{ $attribute }}[]"{{ in_array($device, (array)$project->device_restrictions) ? ' checked' : '' }}>
        ...
        @php($value = old($attribute) ?? $project->qualification_type)
        <div class="form-group row">
        ...
        @php($value = old($attribute) ?? $project->exclusions ?? '')
        <div class="form-group row">
            <label class="col-sm-5 col-lg-4 col-form-label" for="{{ $attribute }}">Exclusion by project</label>

            <div class="col-sm-7 col-lg-8">
                <input type="text" class="form-control{{ $errors->has($attribute) ? ' is-invalid' : '' }}"
                       id="{{ $attribute }}" name="{{ $attribute }}" value="{{ empty($value) ? '' : implode(", ", $value) }}">
        ...
        @php($value = old($attribute) ?? $project->live_link)
        <div class="form-group row">
        ...
        @php($value = old($attribute) ?? $project->test_link)
        <div class="form-group row">
