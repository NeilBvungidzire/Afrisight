...
class MobileNumberBlacklistController extends Controller {
    ...
    public function index() {
        ...
        $blackists = AccountControlService::byMobileNumber()
            ->getBannedQueryBuilder()
            ->paginate($limit);

        return view('admin.account-quality.mobile-number.index', compact('blackists'));
    }
    ...
    public function add() {
        ...
        $input = request()->get('list');

        $list = array_filter(explode("\n", trim($input)));

        $data = [];
        foreach ($list as $number) {
            $number = Str::start($number, "+");
            if ( ! $countryCode = PhoneNumber::make($number)->getCountry()) {
                continue;
            }
        ...
        DB::table('blacklists')->insert($data);
    }
